...
int ep_cert_init_aes_key(ep_client_t * client)
{
	ep_aes_key_t aes;
	char * curtime;
	time_t tm;
	...
	time(&tm);
	curtime = ctime(&tm);
	ep_crypt_md5((unsigned char *)aes.key, (const char *)client, sizeof(client));
	ep_crypt_md5((unsigned char *)aes.vec, curtime, (int)strlen(curtime));
	r = ep_io_update_aes(client->io_mgr, client->fd, &aes);
	assert(r == 1);
...
int ep_cert_client_rsa_sign(ep_client_t * client)
{
	...
	ret = ep_cert_init_aes_key(client);
	if (ret != 1)
	{
        assert(0);
...

	{
		...
		ep_io_get_aes(client->io_mgr, client->fd, &aes);
		...
		ep_pack_append_binary(pack, "EP_CRYPT", ep_buffer_ptr(tmp_buffer), ep_buffer_size(tmp_buffer));
		ep_buffer_free(&tmp_buffer);
	}

	{
		...
		param.category = EP_MAKEWORD(EP_CATEGORY_INTERNAL_RSA, EP_CATEGORY_INTERNAL_RSA_KEY);
	...
	if (ep_req_wait(handle, &pack, client->conf.wait_timeout) == 0)
	{
        assert(0);
...
ep_pack_t * ep_cert_server_rsa_verify(void * opaque, int fd, ep_pack_t * pack, int * b_req_reclaim)
{
	...
	/* server init client EP_CRYPT ok */
	ep_pack_append_int(pack, "result", 1);
